...
// ==========================================
// 2. Deal of the Week Countdown Timer
// ==========================================
    ...
    grid.innerHTML = `
      <div class="col-span-full py-16 text-center">
        ...
        <p class="text-sm text-slate-500 mt-1 max-w-md mx-auto">
          ${isWishlistEmpty ? 'Tap the heart icon on any chair, desk, or accessory to save items here.' : `We couldn't find items matching "${currentSearch}". Try searching for Herman Miller, Standing Desk, or clear your filters.`}
        </p>
        <button onclick="clearFilters()" class="mt-4 btn-primary text-xs">
          ${isWishlistEmpty ? 'Explore All Products' : 'Clear All Filters'}
        </button>
      </div>
    `;
    return;
      ...
      <div class="product-card group" id="card-${product.id}">
        ...
        <div class="product-img-wrap" onclick="openQuickView('${product.id}')" style="cursor: pointer;">
          ...
          <div class="product-actions-overlay" onclick="event.stopPropagation()">
            <button onclick="addToWishlist('${product.id}')" class="btn-action-icon ${isWishlisted ? 'text-red-500 !border-red-400' : ''}" title="${isWishlisted ? 'Remove Wishlist' : 'Add to Wishlist'}" aria-label="Toggle Wishlist">
              <i class="${isWishlisted ? 'fas fa-heart text-red-500' : 'far fa-heart'}"></i>
            </button>
            <button onclick="openQuickView('${product.id}')" class="btn-action-icon" title="Quick View" aria-label="Quick View">
        ...
        <div class="product-content">
          <div class="flex items-center justify-between mb-1">
            ...
            <div class="flex items-center gap-1 text-[11px] text-amber-500 font-bold">
              ...
              <span class="text-slate-400 font-normal hidden sm:inline">(${product.reviewCount})</span>
            </div>
          </div>
...
function openOrderPlacementModal(singleItem = null) {
  if (singleItem) {
    activeOrderItems = [{ ...singleItem, quantity: 1 }];
...
function handleOrderSubmit(e) {
  ...
  message += `━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  message += `Please confirm order receipt, share payment details / QR, and schedule dispatch. Thank you!`;
...
function buyViaWhatsAppItem(productId) {
  const product = PRODUCTS_DATA.find(p => p.id === productId);
  if (!product) return;
  openOrderPlacementModal(product);
}
  ...
  container.innerHTML = `
    ...
    <div class="md:col-span-6">
      ...
      <div class="mt-6 flex flex-wrap gap-3">
        <button onclick="addToCart('${product.id}'); closeQuickView();" class="btn-primary flex-1 justify-center">
          <i class="fas fa-shopping-cart"></i> Add To Cart
        </button>
        <button onclick="buyViaWhatsAppItem('${product.id}')" class="btn-whatsapp flex-1 justify-center">
